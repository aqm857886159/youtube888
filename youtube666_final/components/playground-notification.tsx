...
export function PlaygroundSetupModal({ playgroundInfo, envs }: PlaygroundSetupModalProps) {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-modal flex items-start justify-end p-4">
      {/* Backdrop */}
      <div
      ...
      <button
        ...
        className={cn(
          "pointer-events-auto absolute right-4 top-4 inline-flex h-9 items-center justify-center gap-2 rounded-lg bg-text-primary px-4 text-center text-sm font-medium text-text-on-accent-primary shadow-sm outline-none transition-all duration-200 hover:bg-text-secondary",
          open && "pointer-events-none scale-95 opacity-20",
        )}
      >
        {hasPlaygroundExpiry ? (
          <Clock className="h-4 w-4 text-text-on-accent-primary" />
        ) : (
          <AlertTriangle className="h-4 w-4 text-text-on-accent-primary" />
        )}
        <span className={cn("text-base text-text-on-accent-primary")}>Site Setup</span>
      </button>
      {/* Modal */}
      <div
        className={cn(
          "pointer-events-auto absolute right-4 top-16 flex w-[500px] origin-top flex-col rounded-xl border border-border bg-surface-primary shadow-xl outline-none transition-all duration-200",
          open
